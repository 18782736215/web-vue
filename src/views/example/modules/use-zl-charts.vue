<template>
  <!-- echarts图表组件 使用示例 -->
  <zl-section header="echarts图表组件" type="overspread">
    <zl-charts id="acid-echart1" :option="option" />
  </zl-section>
</template>
...
export default {
  ...
  methods: {
    orderEchart() {
      // this.$echarts.dispose(document.getElementById('acid-echart'))
      // 基于准备好的dom，初始化echarts实例
      // let myEchart = this.$echarts.init(document.getElementById('acid-echart'))
      // myEchart.clear() // 清空重新绘制
      // 数据
      let data1 = [1700, 1597, 1100, 1380, 2289, 1901, 1589, 1790, 2007, 2100, 1380, 2200, 2141, 1351]
      let datas = ['1号', '2号', '3号', '4号', '5号', '6号', '7号', '8号', '9号', '10号', '11号', '12号', '13号', '14号']
      let XName = []
      ...
      datas.map((item, index) => {
        // item = this.yearSecOne(year, index, 'add')
        // XName.unshift(item.substring(5, 10))
        XName.unshift(item)
      })
      XName.map((item, index) => {
        arrTemp[index] = []
        arrTemp[index][0] = item
        arrTemp[index][1] = data1[index]
      })
      // 辅
      let img = [
      ...
      this.option = {
        ...
        series: [{
          ...
          areaStyle: {
            ...
            shadowColor: 'rgba(84, 131, 255, 0.9)',
            shadowBlur: 20
          },
          // smooth: true,
          itemStyle: {
