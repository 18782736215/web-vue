<script>
import { useScopedSlot, messageToHtml, clearHtmlExcludeImg } from '@/plugins/lemon-imui/utils'
...
export default {
  ...
  methods: {
    ...
    saveRangeToLast() {
      ...
      range.selectNodeContents(this.$refs.textarea.value)
      range.collapse(false)
      selection.removeAllRanges()
      selection.addRange(range)
    },
    inertContent(val, toLast = false) {
      if (toLast) this.clipboardUrlsaveRangeToLast()
      this.focusRange()
    ...
    _renderEmojiTabs() {
      const renderImageGrid = items => {
        return items.map(item => <img src={item.src} title={item.title} class='lemon-editor__emoji-item' on-click={() => this._handleSelectEmoji(item)} />)
      }
    ...
    _handleSelectEmoji(item) {
      this.inertContent(`<img emoji-name="${item.name}" src="${item.src}"></img>`)
  ...
  render() {
    ...
    return (
      <div class='lemon-editor'>
        ...
        <input style='display:none' type='file' multiple='multiple' ref='fileInput' accept={this.accept} onChange={this._handleChangeFile} />
        <div class='lemon-editor__tool' style='display:none'>
        ...
        <div class='lemon-editor__inner'>
          <div class='lemon-editor__input' ref='textarea' contenteditable='true' on-keyup={this._handleKeyup} on-keydown={this._handleKeydown} on-paste={this._handlePaste} on-click={this._handleClick} spellcheck='false' />
        </div>
        <div class='lemon-editor__footer'>
          <div class='lemon-editor__tip'>{useScopedSlot(this.IMUI.$scopedSlots['editor-footer'], '使用 ctrl + enter 快捷发送消息')}</div>
          <div class='lemon-editor__submit'>
            <lemon-button disabled={this.submitDisabled} on-click={this._handleSend}>
              {this.sendText}
            </lemon-button>
          </div>
        </div>
