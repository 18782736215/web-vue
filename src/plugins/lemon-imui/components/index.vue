<script>
import { useScopedSlot, funCall, generateUUID, clearHtmlExcludeImg } from '@/plugins/lemon-imui/utils'
import { isFunction, isString, isEmpty } from '@/plugins/lemon-imui/utils/validate'
import contextmenu from '../directives/contextmenu'
import { DEFAULT_MENUS, DEFAULT_MENU_LASTMESSAGES, DEFAULT_MENU_CONTACTS } from '@/plugins/lemon-imui/utils/constant'
import lastContentRender from '../lastContentRender'

import MemoryCache from '@/plugins/lemon-imui/utils/cache/memory'

let allMessages = {} // 存储当前对话框所有消息
const emojiMap = {}
const toPx = val => {
...
export default {
  ...
  computed: {
    ...
    lastMessages() {
      ...
      const data = this.contacts.filter(item => item.id)
      data.sort((a1, a2) => {
  ...
  methods: {
    ...
    _renderWrapper(children) {
      return (
        <div
          ...
          ref='wrapper'
          class={['lemon-wrapper', `lemon-wrapper--theme-${this.theme}`, { 'lemon-wrapper--simple': this.simple }, this.drawerVisible && 'lemon-wrapper--drawer-show']}>
          {children}
        </div>
      )
    },
    ...
    _renderMenuItem() {
      ...
      this.menus.forEach(item => {
        ...
        const node = (
          <div
            class={['lemon-menu__item', { 'lemon-menu__item--active': this.activeSidebar == name }]}
            on-click={() => {
              funCall(click, () => {
      ...
      return this._renderSidebar(
        [
          ...
          this.contacts.map(contact => {
            ...
            const node = [
              contact.index !== prevIndex && <p class='lemon-sidebar__label'>{contact.index}</p>,
              this._renderContact(
                {
                  contact: contact,
                  simple: true
    ...
    _renderSidebar(children, name, fixedTop) {
      return (
        <div class='lemon-sidebar' v-show={this.activeSidebar == name} on-scroll={this._handleSidebarScroll}>
          ...
          <div v-infinite-scroll={this._loadScroll} class='lemon-sidebar__scroll' >{children}</div>
        </div>
      )
    },
    _renderDrawer() {
    ...
    _renderContainer() {
      ...
      nodes.push(
        <div class={cls} v-show={this._menuIsMessages() && defIsShow && curact.id}>
          <div class='lemon-container__title'>{useScopedSlot(this.$scopedSlots['message-title'], <div class='lemon-container__displayname'>{curact.displayName}</div>, curact)}</div>
          <div class='lemon-vessel'>
            <div class='lemon-vessel__left'>
              <lemon-messages ref='messages' loading-text={this.loadingText} loadend-text={this.loadendText} hide-time={this.hideMessageTime} hide-name={this.hideMessageName} time-format={this.messageTimeFormat} reverse-user-id={this.user.id} on-reach-top={this._emitPullMessages} messages={this.currentMessages} />
              <lemon-editor ref='editor' tools={this.editorTools} sendText={this.sendText} sendKey={this.sendKey} wrapKey={this.wrapKey} onSend={this._handleSend} onUpload={this._handleUpload} />
            </div>
            <div class='lemon-vessel__right'>{useScopedSlot(this.$scopedSlots['message-side'], null, curact)}</div>
          </div>
        </div>
      )
      nodes.push(
