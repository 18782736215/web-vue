<script>
import { hoursTimeFormat } from '@/plugins/lemon-imui/utils'
import { isString } from '@/plugins/lemon-imui/utils/validate'
  ...
  render() {
    return (
      <div class='lemon-messages' ref='wrap' on-scroll={this._handleScroll}>
        <div class={['lemon-messages__load', `lemon-messages__load--${this._loadend ? 'end' : 'ing'}`]}>
          <span class='lemon-messages__loadend'>{isString(this.loadendText) ? this.loadendText : this.loadendText()}</span>
          <span class='lemon-messages__loading'>{this.loadingText ? isString(this.loadingText) ? this.loadingText : this.loadingText() : <i class='lemon-icon-loading lemonani-spin' />}</span>
        </div>
